...
- **At 45 FPS:** 20,000 boids (CUDA) vs 600 boids (CPU) — **33.3× improvement**  
- **At 30 FPS:** 28,000 boids (CUDA) vs 700 boids (CPU) — **40× improvement**
...
The CUDA version's logic is mostly the same, just parallelized. The main issue I ran into was with the CUDA compiler — it didn't like me using a simple wrapper class for 3D vectors even though it had no polymorphic behavior. I also had to enable a setting that allowed linking across multiple CUDA files. I'm guessing most people just use one large CUDA file, but I structured the project like I would a typical C++ codebase.
